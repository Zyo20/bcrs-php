        ...
        if (isset($_POST['skip_payment']) && !$requiresPayment) {
            ...
            try {
                ...
                if (!empty($resourceItems)) {
                    foreach ($resourceItems as $index => $resourceId) {
                        // Get quantity for this resource (if it's an equipment)
                        $quantity = 1; // Default quantity
                        if (isset($resourceQuantities[$resourceId])) {
                            $quantity = max(1, (int)$resourceQuantities[$resourceId]); // Ensure at least 1
                        }
                        
                        // Add resource to the reservation
                        $stmt = $db->prepare("INSERT INTO reservation_items (reservation_id, resource_id, quantity) VALUES (?, ?, ?)");
                            ...
                            <div class="flex flex-col md:flex-row items-center">
                                ...
                                <div class="md:w-1/2 text-center">
                                    <div class="bg-white p-2 inline-block rounded">
                                        <img src="includes/maria.jpg" alt="GCash QR Code" class="w-40 h-40 object-cover">
                                    </div>
                                </div>
                            </div>
                        </div>
